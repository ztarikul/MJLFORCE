...
class ShipToPartySeeder extends Seeder
{
    /**
    ...
    public function run(): void
    {
        ...
        // $file1Path = public_path('data/s2ptosh2p_filter.json');
        // $outputPath = public_path('data/relationParty.json');

        // // Decode JSON files
        // $file = json_decode(file_get_contents($file1Path), true);


        //   $result = collect($file)->map(function ($item) {
        //     return [
        //         'Customer_Code' => $item['Customer_Code'] ?? null,
        //         'Ship_to_Party' => $item['Ship_to_Party'] ?? null,
        //     ];
        // })->values()->toArray();
        ...
        $json = file_get_contents(public_path('data/shipToParty.json'));
        $shipToParties = json_decode($json, true);
        $matchedfile = json_decode(file_get_contents(public_path('data/relationParty.json')), true);
        foreach($shipToParties as $data){
            
            $result = collect($matchedfile)->where('Ship_to_Party', trim($data['Ship_to_Party']))->first();
            
            try{
                ShipToParty::create([
                    'sold_to_party_id' => isset($result['Customer_Code']) ? SoldToParty::where('customer_code', $result['Customer_Code'])->first()->id : null,
